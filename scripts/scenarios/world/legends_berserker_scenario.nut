this.legends_berserker_scenario <- this.inherit("scripts/scenarios/world/starting_scenario", {
	...
	function onSpawnAssets()
	{
		...
		this.addScenarioPerk(bros[0].getBackground(), this.Const.Perks.PerkDefs.LegendUberNimble);
		bros[0].getFlags().set("IsPlayerCharacter", true);
		bros[0].setVeteranPerks(2);
		local items = bros[0].getItems();
		items.unequip(items.getItemAtSlot(this.Const.ItemSlot.Mainhand));
		items.unequip(items.getItemAtSlot(this.Const.ItemSlot.Offhand));
		items.equip(this.new("scripts/items/weapons/twinaxes_t3"));
