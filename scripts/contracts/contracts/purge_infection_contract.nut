...
//add by bigmap
	...
	function createScreens()
	{
		...
		this.m.Screens.push({
			ID = "Task",
			Title = "Negotiations",
			Text = "[img]gfx/ui/events/event_31.png[/img]{%employer% throws back his silken sleeves and cracks his knuckles.%SPEECH_ON%Weeks Before a extreme vile threat arose in the form of the Plague of Undeath. To our horror, my senators discovered that the plague was not merely a means of mass murder, but rather the means of turning innocent townsfolk into undead creatures, indicates the coming crisis will be ever harder.%SPEECH_OFF%You nod as though a sellsword is often hired to deal with hard things. The man continues.%SPEECH_ON%My henchman was sent to the town of %settlementname% for an investigation, the grain there had already been distributed, the people of %settlementname% would soon become undead, %noblehousename% are sorry for this but we must do anything we could to prevent further harm.\n\n What I need you to do is burn them all, especially kill those despicable necromancers, puppeteers that hold the strings to make the dead arise, also the ones who spread the horrible infection. To end the assault by these very monsters, I will also send two experienced paladins with you, they are strongly agianst the \'dead\', There\'s a large satchel of crowns waiting for you here if you go there and help defend it. What do you say, hm?%SPEECH_OFF%}",
			Image = "",
			List = [],
			ShowEmployer = true,
			ShowDifficulty = true,
		...
		this.m.Screens.push({
			...
			Text = "[img]gfx/ui/events/event_02.png[/img]{Approaching %settlementname%, %randombrother% suddenly calls out from his lead point.%SPEECH_ON%Sir, hurry!%SPEECH_OFF%You rush over to him and look ahead. The town is absolutely surrounded by a pale sea of bobbing, moaning undead! The %companyname% will have to cut through them if it is to get inside. | A man jogs toward the %companyname%. He\'s holding one of his arms and a crown of crimson runs down his head. He yells out.%SPEECH_ON%Go, go away! There\'s nothing for you here but horrors!%SPEECH_OFF%%randombrother% throws the stranger to the ground and draws a weapon to keep him there. You stay the mercenary\'s hand as you look ahead: %settlementname% is already surrounded by a large number of undead. The %companyname% needs to act fast! | You\'ve arrived just in time: the walls of %settlementname% are already under assault by the undead! | Rounding a pathway, you\'re brought to a sudden stop. Ahead of you, %settlementname% is surrounded by a crowd of undead. Nearer to you, a few linger, oddly stranded from the horde. The %companyname% will need to fight its way into %settlementname%! | The walls of %settlementname% are strangely grey - wait, that\'s not the wood, it\'s the undead! To your horror, the pale monsters are already attacking, but you\'ve time yet to save %settlementname% and fight your way in. Drawing out your sword, you command the %companyname% to battle! | A shapeless formation of undead are already idling outside of %settlementname%\'s walls. You can see the heads of defenders peeking over the defenses, trying hard to not give themselves away. Drawing your sword, you tell the %companyname% that they will have to fight their way into the town. | A few undead are already at the gates of %settlementname%! Guards atop the gatehouse wave at you, put a finger to their lips, then point down. It appears the ghoulish monsters are yet attacking because they\'re unaware? You\'re not sure, you just know that the %companyname% only has one way in and that will be by the sword! | Luckily, you find %settlementname% still standing. Unluckily, the walls are being battered by a crowd of pale undead. The %companyname% will have to fight its way in!}",
			Image = "",
			List = [],
			Options = [
				{
