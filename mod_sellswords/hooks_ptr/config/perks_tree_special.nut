::Const.Perks.SpecialTrees.removeSpecialPerk <- function( _perk )
{
	...
	return find != null ? this.Perks.remove(find) : null;
}
...
::Const.Perks.SpecialTrees.addSpecialPerk(25, ::Const.Perks.PerkDefs.PTRPrimalFear, 6, "Has the talent to spread the fear of death on the battlefield.", function( _player, _chance ) {
	if (!_player.getBackground().hasPerkGroup(::Const.Perks.ViciousTree) && !_player.getBackground().hasPerkGroup(::Const.Perks.DeviousTree) && !_player.getBackground().hasPerkGroup(::Const.Perks.UnstoppableTree) && !_player.getBackground().hasPerkGroup(::Const.Perks.LargeTree))
	{
		return 0;
	}

	if (!_player.getBackground().hasPerkGroup(::Const.Perks.FlailTree) && !_player.getBackground().hasPerkGroup(::Const.Perks.AxeTree) && !_player.getBackground().hasPerkGroup(::Const.Perks.CrossbowTree) && !_player.getBackground().hasPerkGroup(::Const.Perks.PolearmTree))
	{
		return 0;
	}
...
::Const.Perks.SpecialTrees.addSpecialPerk(1, ::Const.Perks.PerkDefs.crmeleedefense, 7, "Born to Be Surrounded", function( _player, _chance ) {
