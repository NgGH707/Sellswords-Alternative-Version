	...
	foreach( t in p.Troops )
	{
		...
		for( local i = 0; i != t.Num; i = i )
		{
			...
			if (unit.Variant > 0)
			{
				...
				if (!this.Const.DLC.Wildmen || this.Math.rand(1, upperBound) > unit.Variant + mb + (this.World.getTime().Days > 100 ? 0 : -1) + (::Legends.Mod.ModSettings.getSetting("RogueMode").getValue() ? -100 : 0))
				{
					unit.Variant = 0;
				}
				else
...
::Const.World.Common.addTroop = function ( _party, _troop, _updateStrength = true, _minibossify = 0 )
{
	...
	if (troop.Variant > 0)
	{
		_minibossify = _minibossify + this.World.Assets.m.ChampionChanceAdditional + (::Legends.Mod.ModSettings.getSetting("RogueMode").getValue() ? -100 : 0);
